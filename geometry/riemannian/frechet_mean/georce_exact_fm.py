...
class GEORCE_ExactFM(ABC):
    def __init__(self,
                 M:RiemannianManifold,
                 Geodesic:Callable[[Array, Array], Array],
    ...
    def Denergy(self,
        ...
        return grad(self.energy, argnums=0)(z_mu,*args)
    ...
    def cond_fun(self, 
        ...
        #mu = self.M.f(z_mu)
        #grad_norm = jnp.linalg.norm(self.Dobj(mu))
    ...
    def while_step(self,
        ...
        zt, ut = vmap(self.update_geodesic, in_axes=(0,None))(self.z_obs, z_mu)
        ...
        zt_hat, ut_hat = vmap(self.update_geodesic, in_axes=(0,None))(self.z_obs, z_mu_hat)

        tau = self.line_search((z_mu,), zt, z_mu, z_mu_hat, ut_hat, ut)
        ...
        grad_norm = jnp.linalg.norm(self.Denergy_frechet(ut, Gt))
    ...
    def __call__(self, 
        ...
        self.z_obs = z_obs
        self.x_obs = vmap(self.M.f)(self.z_obs)
        ...
        if step == "while":
            ...
            zt, ut = vmap(self.update_geodesic, in_axes=(0,None))(self.z_obs, z_mu_init)
            gt, Gt = self.gt(zt, ut[:,1:])
            ...
            zt, ut, z_mu, gt, gt_inv, grad_norm, idx = lax.while_loop(self.cond_fun, 
                                                           self.while_step, 
                                                           init_val=(zt, ut, z_mu_init, gt, 
                                                                     gt_inv, grad_norm, 0),
                                                           )
