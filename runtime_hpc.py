...
def riemannian_runtime(method_name:str, N_data:int)->None:
    ...
    batch_size = int(args.batch_size*N_data)
    ...
    if method_name == "euclidean":
        ...
        try:
            if "ADA" in method_name:
                optimizer = getattr(optimizers, method.replace("ADA", ""))
                ...
                methods[method_name] = estimate_method(jit(lambda z: FrechetMean(z,batch_size)), 
                                                       jit(Geodesic), 
                                                       z_obs, 
                                                       M,
                                                       )
            else:
                optimizer = getattr(optimizers, method_name)
                ...
                methods[method_name] = estimate_method(jit(FrechetMean), jit(Geodesic), z_obs, M)
        except:
            try:
                ...
                methods[method_name] = estimate_method(FrechetMean, jit(Geodesic), z_obs, M)
            except:
                raise ValueError(f"Method, {method_name}, not defined!")
...
def finsler_runtime(method_name:str, N_data:int)->None:
    ...
    if method_name == "euclidean":
        ...
        try:
            if "ADA" in method_name:
                optimizer = getattr(optimizers, method_name.replace("ADA", ""))
...
def lorentz_runtime(method_name:str, N_data:int)->None:
    ...
    if method_name == "euclidean":
        ...
        try:
            if "ADA" in method_name:
                ...
                methods[method_name] = estimate_lorentz(jit(lambda t,z: FrechetMean(t,z,batch_size)), 
                                                        jit(Geodesic), 
                                                        z_obs, 
                                                        M,
                                                        )
                ...
                methods[method_name] = estimate_lorentz(jit(FrechetMean), jit(Geodesic), z_obs, M)
        except:
            try:
                ...
                methods[method_name] = estimate_lorentz(FrechetMean, jit(Geodesic), z_obs, M)
            except:
                raise ValueError(f"Method, {method_name}, not defined!")
    save_times(methods, save_path)
...
if __name__ == '__main__':
    ...
    if args.geometry == "Riemannian":
        for m in methods:
            riemannian_runtime(m, N_data)
    elif args.geometry == "Finsler":
        for m in methods:
            finsler_runtime(m, N_data)
    elif args.geometry == "Lorentz":
        for m in methods:
            lorentz_runtime(m, N_data)
    else:
